 ...
 */
function hm_phpthumb_it( $url, $args = array() ) {
	...
	if ( !file_exists( $new_filepath )  || $cache === false || $ext == '.gif') :
		...
		if ( $ext == '.gif' ) :
			...
			return hm_phpthumb_it( $new_filepath . '.jpg', $args );
...
function phpthumb_image_from_args( $image_path, $args ) {
	...
	$image = hm_phpthumb_it( $image_path, $args );
 ...
 * @return array
 */
function hm_get_attached_images( $post = null, $return = 'file' ) {
	if ( $post === null )
		global $post;
...
function wm_watermark_preview_image( $position, $padding, $image_id ) {
	...
	return '<img src="' . hm_phpthumb_it( $image, 200, 0, false, true, $watermark, false ) . '" /><a target="_blank" href="' . tw_phpthumb_it( $image, 1000, 0, false, true, $watermark, false ) . '">View Large</a>';
}
